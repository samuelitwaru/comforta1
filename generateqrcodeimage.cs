...
namespace GeneXus.Programs {
   public class generateqrcodeimage : GXProcedure
   {
      ...
      public void executeSubmit( string aP0_email ,
                                 out string aP1_linkURL )
      {
         generateqrcodeimage objgenerateqrcodeimage;
         objgenerateqrcodeimage = new generateqrcodeimage();
         objgenerateqrcodeimage.AV19email = aP0_email;
         objgenerateqrcodeimage.AV8linkURL = "" ;
         objgenerateqrcodeimage.context.SetSubmitInitialConfig(context);
         objgenerateqrcodeimage.initialize();
         Submit( executePrivateCatch,objgenerateqrcodeimage);
         aP1_linkURL=this.AV8linkURL;
      }
      ...
      void executePrivateCatch( object stateInfo )
      {
         try
         {
            ((generateqrcodeimage)stateInfo).executePrivate();
         }
         catch ( Exception e )
         {
